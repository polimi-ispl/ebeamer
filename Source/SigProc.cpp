...
void freqToTime(AudioBuffer<float>& time, const int timeCh, const CpxVec& freq, const juce::dsp::FFT* fft, const Vec& window, float alpha){
    ...
    if (alpha < 1){
        ...
        FloatVectorOperations::addWithMultiply(time.getWritePointer(timeCh), tmp.getReadPointer(0), alpha, time.getNumSamples());
    }else{
